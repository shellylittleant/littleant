...
<p align="center">
  把自然语言意图，通过双执行架构（循环 + 线性）与机械验证，编译为可执行、可核查、可恢复的服务器动作。
</p>
...
| **失败处理** | 崩溃或问用户 | 三级恢复（L1重试 → L2诊断 → L3重设计） |
| **执行模型** | 一次性生成计划 | 双执行：查询走循环，创建/修改走线性 |
| **透明度** | 黑箱 | 完整执行树，每步有日志 |
...
Claude 走 Anthropic 原生 Messages API；其余服务商走 OpenAI 兼容端点，因此任何 OpenAI 兼容的 API 也都可以用。安装时选一个即可。
...
LittleAnt 以高权限用户执行命令，因此默认是锁定的。**第一个**给机器人发消息的 Telegram 用户会被自动注册为唯一管理员；其他人一律拒绝，需由现有管理员添加：

```
/addadmin <chat_id>
```

管理员列表保存在 `littleant/config.json` 的 `admin_chat_ids` 字段。
...
| /status | 查看任务进度 |
| /model | 切换 AI 服务商/模型 |
| /addadmin `<chat_id>` | 添加管理员（仅管理员可用） |
| /cancel | 取消当前任务 |

### 一个任务是怎么执行的（创建/修改 → 线性模型）

```
1. 你说："帮我装个 WordPress"
2. 一次性授权：[确认] [自动执行] [修改] [取消] → 你确认
3. 后台AI扫描真实系统状态（已装的包、端口、配置）
4. AI 一次性给出计划：前置命令、要创建/修改的文件、后置命令
5. 批量生成并写入文件；命令执行，失败时走三级恢复
6. 最终机械验证：每个文件存在且非空、每条后置命令的 verify 返回 0
7. 全部通过才汇报"完成"——否则明确告诉你哪一步失败了
```

查询/诊断类任务走循环模型（扫描 → 判断 → 行动 → 重复）。
...
│   │   ├── orchestrator.py   # 循环+线性引擎 + 三级恢复（L1/L2/L3）
...
- 任务是否成功由机械验证决定——只有文件和后置命令真正验证通过才算"完成"，不是 AI 说了算
- 失败的节点从链中**删除**，不是标记——执行自动继续
- 机械验证成功的项目自动保存到模板库——下次 AI 直接复用
...
<p align="center">
  <sub>双执行模型 + 机械复核 + 三级恢复。零依赖，零信任 AI 输出。</sub>
</p>
