...
<p align="center">
  把自然语言意图，通过递归分解，编译为可执行、可核查、可恢复的服务器动作。
</p>
...
- **后台 AI** — 只输出 JSON，无记忆。递归分解任务，生成可执行命令，自主处理失败。拥有完整读写权限。
...
│   ├── core/
│   │   ├── decomposer.py     # 递归分解引擎
│   │   ├── executor.py       # 命令执行器
│   │   ├── verifier.py       # 8 种机械复核器
│   │   ├── recovery.py       # AI 自主恢复
│   │   ├── orchestrator.py   # 主调度器 + 工具库
...
<p align="center">
  <sub>递归分解 + 机械复核。零依赖，零信任 AI 输出。</sub>
</p>
